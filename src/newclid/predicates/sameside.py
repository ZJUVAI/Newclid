...
class SameSide(Predicate):
    ...
    def preparse(cls, args: tuple[str, ...]):
        ...
        if a == b or b == c or a == c or x == y or y == z or x == z:
            return None
        p1 = min(
            (a, b, c), (a, c, b), key=lambda pair: [cls.custom_key(arg) for arg in pair]
        )
        p2 = min(
            (x, y, z), (x, z, y), key=lambda pair: [cls.custom_key(arg) for arg in pair]
        )
        return min(
            p1 + p2, p2 + p1, key=lambda pair: [cls.custom_key(arg) for arg in pair]
        )
