...
class GeometryEquivalenceAnalyzer:
    ...
    def __init__(self):
        ...
        self.CONSTRUCT_EQUIVALENCES = [
            ...
            (['perp', 'cong', 'para'], 'a b c d', lambda args: [
                perp
                for perp in self.generate_2lines_equivalences(args[0:4])
            ]),
    ...
    def generate_2lines_equivalences(self, lines):
        """Generate all equivalent representations of two lines [a,b,c,d]"""
        try:
            assert len(lines) == 4, "Expected exactly 4 elements for two lines"
    ...
    def are_same_figure(self, figure1, figure2):
        ...
        possible_mappings = self.find_point_mapping(components1, components2)

        # Step 4: Check if all components match under the point mapping
        for mapping in possible_mappings:
            if self.check_components_match(components1, components2, mapping):
                return True
    ...
    def update_mapping_for_construct(self, comp1, comp2, current_mapping):
        ...
        if param_format == '*points':
            ...
            for perm in equiv_rule(points1):
                ...
                if valid:
                    new_possible_mappings.append(new_mapping)
    ...
    def find_point_mapping(self, components1, components2):
        ...
        for construct_type in unique_constructs:
            ...
            if comp1 and comp2:
                new_possible_mappings = []
                for mapping in possible_mappings:
                    updated_mappings = self.update_mapping_for_construct(comp1, comp2, mapping)
                    new_possible_mappings.extend(updated_mappings)
                possible_mappings = new_possible_mappings

        # Ensure all possible mappings have the same length
        if possible_mappings:
            mapping_lengths = [len(m) for m in possible_mappings]
            try:
                assert all(l == mapping_lengths[0] for l in mapping_lengths), "All mappings in possible_mappings must have the same length"
            except AssertionError:
                return None

        new_possible_mappings = []
        for mapping in possible_mappings:
            # Try to complete the mapping with the remaining points
            completed_mappings = self.complete_mapping(components1, components2, mapping, points1, points2)
            new_possible_mappings.extend(completed_mappings)

        try:
            for mapping in new_possible_mappings:
                assert len(mapping) == len(points1), "Mapping must cover all points"
        except AssertionError:
            return None
    ...
    def complete_mapping(self, components1, components2, partial_mapping, points1, points2):
        ...
        try:
            assert len(unmapped_points1) == len(unmapped_points2), "Unmapped points should be equal in both figures"
        except AssertionError:
            return []
        ...
        # Try all permutations of unmapped points
        possible_mappings = []
        for perm in permutations(unmapped_points2, len(unmapped_points1)):
            test_mapping = mapping.copy()
            for p1, p2 in zip(unmapped_points1, perm):
                test_mapping[p1] = p2
            if self.is_mapping_consistent(components1, components2, test_mapping):
                possible_mappings.append(test_mapping)
    ...
    def is_args_equivalent(self, construct_type, args1, args2):
        ...
        if construct_type in ['aconst', 's_angle', 'lconst', 'rconst']:
            # 比较前面的点参数
    ...
    def process_geometry_block(self, data_num, content):
        ...
        if not target_match:
            print(f"⚠️ Data {data_num} missing target construct ({after_q}), skipping")
            return
    ...
    def analyze_input_file(self, input_file):
        """Read and analyze geometry figures from the input CSV file."""
                ...
                for i in range(len(sorted_nums)):
                    ...
                    for j in range(i + 1, len(sorted_nums)):
                        ...
                        if self.are_same_figure(fig1, fig2):
                            ...
                            # goal_predicate1 = fig1.split('?')[1].strip()
                            # goal_predicate2 = fig2.split('?')[1].strip()
                            # print(f"{a}, {b}; {goal_predicate1} , {goal_predicate2}")

        self.number_of_remaining_rows = self.total_rows - len(equivalent_pairs)
        return equivalent_pairs

    def write_results_to_file(self, output_file, equivalent_pairs):
        """Write the results to the output file."""
        print(f"Total rows: {self.total_rows}")
        print(f"Rows to be kept: {self.number_of_remaining_rows}")
        print(f"Ratio of kept rows: {self.number_of_remaining_rows / self.total_rows * 100:.2f}%")
        ...
        try:
            with open(output_path, "w", encoding="utf-8") as out_file:
                out_file.write(f"Total rows: {self.total_rows}\n")
                out_file.write(f"Rows to be kept: {self.number_of_remaining_rows}\n")
                out_file.write(f"Ratio of kept rows: {self.number_of_remaining_rows / self.total_rows * 100:.2f}\n")
                out_file.write(f"Time taken: {time.time() - self.start_time:.2f} seconds\n")
    ...
    def run_analysis(self, input_file="input.jsonl", output_file="output.txt", new_file="new.jsonl"):
        ...
        print("Identifying equivalent figures...")
...
def main():
    input_filename = "example.jsonl" 
    if len(sys.argv) > 1:
        input_filename = sys.argv[1]
    output_filename = f"analysis_deduplicated_by_remap_{input_filename.split('.')[0]}_output.txt"

    analyzer = GeometryEquivalenceAnalyzer()
    analyzer.run_analysis(input_filename, output_filename, new_file="deduplicated_by_remap_" + input_filename)
