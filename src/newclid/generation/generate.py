    ...
    def process_single_problem(self, args: tuple) -> tuple[list, dict]:
        ...
        try:
            ...
            for goal in possible_goals:
                ...
                nl_solution = write_proof_steps(
                    solver_new.proof, print_output=False)
                llm = self.llm_solution(
                    problem_new, aux_points, solver_new.proof)
                llm_remamed = self.llm_solution_renamed(
                    problem_new, aux_points, solver_new.proof)

                if len(aux_points) > 0 and not self.filter.aux_predicates_valid_check(llm['llm_output']):
                    continue
                if len(aux_points) > 0 and not self.filter.aux_predicates_valid_check(llm_remamed['llm_output']):
                    continue
