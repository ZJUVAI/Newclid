...
def _default_sets() -> dict[str, list[str]]:
    ...
    if not isinstance(sets, dict):
        raise ValueError("Default construction config field 'construction_sets' must be a dict.")
...
def _expand_set_names(
    ...
    if not isinstance(set_names, list) or not all(isinstance(v, str) for v in set_names):
        raise ValueError(f"Construction config field '{step_key}' must be a list of set names.")

    expanded: list[str] = []
    seen: set[str] = set()
    for set_name in set_names:
        if set_name not in all_sets:
            raise ValueError(f"Construction config references unknown construction set '{set_name}'.")
        for construction in all_sets[set_name]:
            if available_constructions is not None and construction not in available_constructions:
                raise ValueError(
    ...
    if not expanded:
        raise ValueError(f"Construction config expands to an empty pool for '{step_key}'.")
...
def resolve_construction_config(
    ...
    sets = config.get('construction_sets', {})
    if not isinstance(sets, dict):
        raise ValueError("Construction config field 'construction_sets' must be a dict.")
    sets = _copy_named_lists(sets)
    for set_name, values in sets.items():
        if not isinstance(values, list) or not all(isinstance(v, str) for v in values):
            raise ValueError(f"Construction set '{set_name}' must be a list of strings.")

    missing = [key for key in STEP_KEYS if key not in config]
    if missing:
        raise ValueError(f"Construction config is missing required keys: {', '.join(missing)}.")
