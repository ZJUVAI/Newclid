...
def match_eqratio_eqratio_eqratio(
    ...
    for m1 in proof.symbols_graph.type2nodes[gm.Value]:
        for m2 in proof.symbols_graph.type2nodes[gm.Value]:
            rats1 = []
            ...
            for (l1, l12, l2), (l3, l34, l4) in comb.comb2(pairs):
                ...
                # d2 - d12 = d4 - d34 = m2
                # => d2 - d1 = d4 - d3 (= m1+m2)
                a, b = proof.two_points_of_length(l1)
                c, d = proof.two_points_of_length(l12)
                m, n = proof.two_points_of_length(l3)
                p, q = proof.two_points_of_length(l34)
                # eqangle a b c d m n p q
                e, f = proof.two_points_of_length(l2)
                r, u = proof.two_points_of_length(l4)
...
def match_eqangle_eqangle_eqangle(
    ...
    for m1 in proof.symbols_graph.type2nodes[gm.Measure]:
        for m2 in proof.symbols_graph.type2nodes[gm.Measure]:
            angs1 = []
            ...
            for (d1, d12, d2), (d3, d34, d4) in comb.comb2(pairs):
                ...
                # d2 - d12 = d4 - d34 = m2
                # => d2 - d1 = d4 - d3
                a, b = proof.two_points_on_direction(d1)
                c, d = proof.two_points_on_direction(d12)
                m, n = proof.two_points_on_direction(d3)
                p, q = proof.two_points_on_direction(d34)
                # eqangle a b c d m n p q
                e, f = proof.two_points_on_direction(d2)
                r, u = proof.two_points_on_direction(d4)
                yield dict(zip("abcdefmnpqru", [a, b, c, d, e, f, m, n, p, q, r, u]))
...
def match_perp_perp_npara_eqangle(
    ...
    dpairs = []
    for ang in proof.vhalfpi.neighbors(gm.Angle):
        d1, d2 = ang.directions
        if d1 is None or d2 is None:
            continue
        dpairs.append((d1, d2))
    ...
    for (d1, d2), (d3, d4) in comb.comb2(dpairs):
        a, b = proof.two_points_on_direction(d1)
        c, d = proof.two_points_on_direction(d2)
        m, n = proof.two_points_on_direction(d3)
        p, q = proof.two_points_on_direction(d4)
        if proof.check_npara([a, b, m, n]):
            if ({a, b}, {c, d}) == ({m, n}, {p, q}):
                continue
            if ({a, b}, {c, d}) == ({p, q}, {m, n}):
                continue
...
def match_circle_coll_eqangle_midp(
    ...
    for p, a, b, c in proof.all_circles():
        ...
        for angp in pb.val.neighbors(gm.Angle):
            if not proof.is_equal(anga, angp):
                continue

            _, d = angp.directions
            for line_neighbor in d.neighbors(gm.Line):
                l_points = line_neighbor.neighbors(gm.Point, return_set=True)
...
def match_midp_perp_cong(
    ...
    for m, a, b in proof.all_midps():
        ...
        for line_neighbor in m.neighbors(gm.Line):
            if proof.check_perpl(line_neighbor, ab):
...
def match_cyclic_eqangle_cong(
    ...
    for c in proof.symbols_graph.type2nodes[gm.Circle]:
        ps = c.neighbors(gm.Point)
        for (a, b, c), (x, y, z) in comb.comb2(list(comb.perm3(ps))):
            if {a, b, c} == {x, y, z}:
                continue
            if proof.check_eqangle([c, a, c, b, z, x, z, y]):
...
def match_circle_eqangle_perp(
    ...
    for p, a, b, c in proof.all_circles():
        ...
        for ang in ab.val.neighbors(gm.Angle):
            if proof.is_equal(ang, c_ang):
                _, d = ang.directions
                for line_neighbor in d.neighbors(gm.Line):
                    if a not in line_neighbor.neighbors(gm.Point):
                        continue
...
def match_circle_perp_eqangle(
    ...
    for p, a, b, c in proof.all_circles():
        ...
        if pa.val is None:
            continue
        for line_neighbor in a.neighbors(gm.Line):
            if proof.check_perpl(pa, line_neighbor):
                x = diff_point(line_neighbor, a)
                if x is not None:
                    yield dict(zip("OABCX", [p, a, b, c, x]))
...
def match_perp_perp_ncoll_para(
    ...
    for x, ys in d2d.items():
        if len(ys) < 2:
            continue
        c, d = proof.two_points_on_direction(x)
        for y1, y2 in comb.comb2(ys):
            a, b = proof.two_points_on_direction(y1)
            e, f = proof.two_points_on_direction(y2)
...
def match_eqangle6_ncoll_cong(
    ...
    for a in proof.symbols_graph.type2nodes[gm.Point]:
        for b, c in comb.comb2(proof.symbols_graph.type2nodes[gm.Point]):
            if a == b or a == c:
                continue
            if proof.check_eqangle([b, a, b, c, c, b, c, a]):
                if proof.check_ncoll([a, b, c]):
...
def match_eqangle_perp_perp(
    ...
    for ang in proof.vhalfpi.neighbors(gm.Angle):
        ...
        if d1 is None or d2 is None:
            continue
        for d3, d4 in comb.comb2(proof.symbols_graph.type2nodes[gm.Direction]):
            if d1 == d3 or d2 == d4:
                continue
            ...
            if a13 is None or a31 is None or a24 is None or a42 is None:
                continue
            if proof.is_equal(a13, a24) and proof.is_equal(a31, a42):
                a, b = proof.two_points_on_direction(d1)
                c, d = proof.two_points_on_direction(d2)
                m, n = proof.two_points_on_direction(d3)
                p, q = proof.two_points_on_direction(d4)
...
def match_eqangle_ncoll_cyclic(
    ...
    for l1, l2, l3, l4 in proof.all_eqangles_distinct_linepairss():
        ...
        if not proof.check_ncoll([a, b, p, q]):
            continue
...
def match_eqangle_para(
    ...
    for measure in proof.symbols_graph.type2nodes[gm.Measure]:
        ...
        for d1, d2s in d12.items():
            ...
            for d2, d3 in comb.comb2(d2s):
                c, d = proof.two_points_on_direction(d2)
                e, f = proof.two_points_on_direction(d3)
...
def match_cong_cong_cong_cyclic(
    ...
    for lenght in proof.symbols_graph.type2nodes[gm.Length]:
...
def match_cong_cong_cong_ncoll_contri(
    ...
    for a, b, p, q in g_matcher("cong"):
        for c in proof.symbols_graph.type2nodes[gm.Point]:
            for r in proof.symbols_graph.type2nodes[gm.Point]:
                if any([x in record for x in rotate_simtri(a, b, c, p, q, r)]):
                    continue
                if not proof.check_ncoll([a, b, c]):
                    continue
                if proof.check_cong([b, c, q, r]) and proof.check_cong([c, a, r, p]):
    ...
    for a, b, p, q in g_matcher("cong"):
        for c in proof.symbols_graph.type2nodes[gm.Point]:
            ...
            for r in proof.symbols_graph.type2nodes[gm.Point]:
                ...
                if not proof.check_cong([b, c, q, r]):
                    continue
                if not proof.check_ncoll([a, b, c]):
                    continue
                ...
                if geosolver.numerical.check.same_clock(
                    ...
                    if proof.check_eqangle([b, a, b, c, q, p, q, r]):
                        record.add((a, b, c, p, q, r))
                        yield dict(zip("ABCPQR", [a, b, c, p, q, r]))
                else:
                    if proof.check_eqangle([b, a, b, c, q, r, q, p]):
                        record.add((a, b, c, p, q, r))
                        yield dict(zip("ABCPQR", [a, b, c, p, q, r]))
    ...
    for b, a, b, c, q, p, q, r in enums:
        ...
        if geosolver.numerical.check.same_clock(
            ...
            if proof.check_eqangle([b, a, b, c, q, p, q, r]):
                record.add((a, b, c, p, q, r))
                yield dict(zip("ABCPQR", [a, b, c, p, q, r]))
        elif proof.check_eqangle([b, a, b, c, q, r, q, p]):
            record.add((a, b, c, p, q, r))
            yield dict(zip("ABCPQR", [a, b, c, p, q, r]))
...
def match_eqangle6_eqangle6_ncoll_simtri(
    ...
    for b, a, b, c, q, p, q, r in enums:
        ...
        if any([x in record for x in rotate_simtri(a, b, c, p, q, r)]):
            continue
        if not proof.check_eqangle([c, a, c, b, r, p, r, q]):
            continue
        if not proof.check_ncoll([a, b, c]):
            continue
...
def match_eqratio6_eqratio6_ncoll_simtri(
    ...
    for b, a, b, c, q, p, q, r in enums:
        ...
        if any([x in record for x in rotate_simtri(a, b, c, p, q, r)]):
            continue
        if not proof.check_eqratio([c, a, c, b, r, p, r, q]):
            continue
        if not proof.check_ncoll([a, b, c]):
            continue
...
def match_eqangle6_eqangle6_ncoll_simtri2(
    ...
    for b, a, b, c, q, r, q, p in enums:
        ...
        if any([x in record for x in rotate_simtri(a, b, c, p, q, r)]):
            continue
        if not proof.check_eqangle([c, a, c, b, r, q, r, p]):
            continue
        if not proof.check_ncoll([a, b, c]):
            continue
...
def match_eqangle6_eqangle6_ncoll_cong_contri(
    ...
    record = set()
    for b, a, b, c, q, p, q, r in enums:
        if not proof.check_cong([a, b, p, q]):
            continue
        if (a, b, c) == (p, q, r):
            continue
...
def match_eqratio6_coll_ncoll_eqangle6(
    ...
    for b, d, c in g_matcher("coll"):
        for a in proof.symbols_graph.all_points():
            if proof.check_coll([a, b, c]):
                continue
            if (a, b, d, c) in records or (a, c, d, b) in records:
                continue
            records.add((a, b, d, c))

            if proof.check_eqratio([d, b, d, c, a, b, a, c]):
...
def match_eqangle6_coll_ncoll_eqratio6(
    ...
    for b, d, c in g_matcher("coll"):
        for a in proof.symbols_graph.all_points():
            ...
            if proof.check_eqangle([a, b, a, d, a, d, a, c]):
                yield dict(zip("abcd", [a, b, c, d]))
...
def match_all(name: str, proof: "Proof") -> Generator[tuple[gm.Point, ...], None, None]:
...
def match_generic(
    ...
    for mapping in try_to_map([(c, clause2enum[c]) for c in clauses], {}):
        ...
        for check in numerical_checks:
            ...
            if check.name == "ncoll":
                checks_ok = proof.check_ncoll(args)
            elif check.name == "npara":
                checks_ok = proof.check_npara(args)
            elif check.name == "nperp":
                checks_ok = proof.check_nperp(args)
            elif check.name == "sameside":
                checks_ok = proof.check_sameside(args)
            if not checks_ok:
                break
        if not checks_ok:
            continue
...
def match_all_theorems(
    proof: "Proof", theorems: list[pr.Theorem], goal: pr.Clause
