    ...
    for l0 in _get_lines_thru_all(*points):
        for line in l0.equivs():
            if all([p in line.edge_graph for p in points]):
                ...
                why = line.why_connected_to(colls)
                return line, why
...
class SymbolsGraph:
    def __init__(self) -> None:
        ...
        self.rconst: Dict[
            Tuple[int, int], RatioValue
        ] = {}  # contains all constant ratios
        self.aconst: Dict[
            Tuple[int, int], AngleValue
        ] = {}  # contains all constant angles.
        self.lconst: Dict[float, Length] = {}  # contains all constant lengths.
        self.halfpi, _ = self.get_or_create_const_ang(1, 2)
        self.vhalfpi = self.get_or_create_node_val(self.halfpi, [])

    def connect(self, a: Symbol, b: Symbol, deps: list["Dependency"]) -> None:
        a.connect_to(b, deps)
        b.connect_to(a, deps)
    ...
    def get_or_create_node_val(self, node: Symbol, deps: list["Dependency"]) -> Symbol:
        """Get a node value (equality) node, creating it if necessary."""
        if node.val:
            return node.val

        if type(node) not in NODES_VALUES:
            return node
        ...
        self.connect(node, v, deps)
        return v

    def make_equal(self, x: Symbol, y: Symbol, deps: list[Dependency]) -> None:
        """Make that two nodes x and y are equal, i.e. merge their value node."""

        vx = self.get_or_create_node_val(x, [])
        vy = self.get_or_create_node_val(y, [])
        ...
        # If eqangle on the same directions switched when they are perpendicular
        if (
            isinstance(x, Angle)
    ...
    def merge(self, nodes: list[Symbol], deps: list["Dependency"]) -> Symbol:
        """Merge all nodes."""
        if len(nodes) < 2:
            return
        ...
        for node in nodes:
            ...
            raise ValueError("no node present in the symbols graph")

        node0.merge(nodes1, deps)
        self.remove([n for n in nodes1 if n.rep() != n])
        if type(node0) in NODES_VALUES:
            self.merge([self.get_or_create_node_val(n, []) for n in nodes], [])

    def remove(self, nodes: list[Symbol]) -> None:
        """Remove nodes out of self because they are merged."""
        for node in nodes:
            try:
                self.type2nodes[type(nodes[0])].remove(node)
            except ValueError:
                pass
    ...
    ) -> tuple[Optional[Angle], Optional[Angle]]:
    ...
    def get_circles(self, *points: list[Point]) -> list[Circle]:
        """
        get circles on which are the given points
        """
        points = set(points)
    ...
    def _create_const_ang(self, n: int, d: int) -> None:
        n, d = simplify(n, d)
        n %= d
        self.aconst[(n, d)] = self.new_node(AngleValue, f"{n}pi/{d}")

    def _create_const_rat(self, n: int, d: int):
        n, d = simplify(n, d)
        self.rconst[(n, d)] = self.new_node(RatioValue, f"{n}/{d}")

    def get_or_create_const_ang(self, n: int, d: int) -> tuple[AngleValue, AngleValue]:
        n, d = simplify(n, d)
        n %= d
    ...
    def get_or_create_const_rat(self, n: int, d: int) -> tuple[RatioValue, RatioValue]:
        n, d = simplify(n, d)
    ...
    def get_or_create_const_length(self, length) -> Length:
        ...
        if length not in self.lconst:
            ...
            self.get_or_create_node_val(length_node, [])
    ...
    def get_or_create_const(
        ...
        raise NotImplementedError(
            "Unsupported construction for constants: %s", construction_name
        )

    def get_or_create_segment(
        self, p1: Point, p2: Point, deps: list["Dependency"]
    ) -> Segment:
        ...
        if p1.name > p2.name:
            p1, p2 = p2, p1
        s = self.new_node(Segment, name=f"[{p1.name}{p2.name}]")
        self.connect(p1, s, deps)
        self.connect(p2, s, deps)
        s.points = {p1, p2}
        return s

    def get_or_create_angle_from_lines(
        self, l1: Line, l2: Line, deps: list["Dependency"]
    ) -> tuple[Angle, Angle]:
        return self.get_or_create_angle_from_directions(
            self.get_or_create_node_val(l1, []),
            self.get_or_create_node_val(l2, []),
            deps,
        )

    def get_or_create_angle_from_directions(
        self, d1: Direction, d2: Direction, deps: list["Dependency"]
    ) -> tuple[Angle, Angle]:
        """Get or create an angle between two Direction d1 and d2."""
        for a in self.type2nodes[Angle]:
            if a.directions == (d1.rep(), d2.rep()):
                return a, a.opposite
        ...
        self.connect(d2, a12, deps)
        self.connect(d1, a21, deps)
    ...
    def get_or_create_ratio_from_segments(
        self, s1: Segment, s2: Segment, deps: list["Dependency"]
    ) -> tuple[Ratio, Ratio]:
        return self.get_or_create_ratio_from_lengths(
            self.get_or_create_node_val(s1.val, []),
            self.get_or_create_node_val(s2.val, []),
            deps,
        )

    def get_or_create_ratio_from_lengths(
        self, l1: Length, l2: Length, deps: list["Dependency"]
    ) -> tuple[Ratio, Ratio]:
    ...
    def get_new_line_thru_pair(self, p1: Point, p2: Point) -> Line:
        ...
        self.connect(p1, line, [])
        self.connect(p2, line, [])
        self._pair2line[(p1, p2)] = line
        return line

    def get_or_create_line_thru_pair(self, p1: Point, p2: Point) -> Line:
    ...
    def get_or_create_line_thru_pair_why(
    ...
    def get_new_circle_thru_triplet(self, p1: Point, p2: Point, p3: Point) -> Circle:
        ...
        self.connect(p1, circle, [])
        self.connect(p2, circle, [])
        self.connect(p3, circle, [])
    ...
    def two_points_on_direction(d: Direction) -> tuple[Point, Point]:
        neighbors = list(d.neighbors(Line))
        line_neighbor = neighbors[0]
        p1, p2 = list(line_neighbor.neighbors(Point))[:2]
        return p1, p2
...
def is_equal(x: Symbol, y: Symbol) -> bool:
    return (x.val or x).rep() == (y.val or y).rep()
