...
最后更新：2026-06-02 | 当前 commit: `750d5feba8ef7daeeea41b1341e58618e6be65c1`
...
| v17 checkpoint-500 | `qwen3_vl_text` multiaux | 13/16 | 56/95 | 5k `bucket_unified`，ddar bug fix 后重跑（20260524，eval commit `42288da`） |
...
| v27 checkpoint-500 | `qwen3_vl_text` multiaux | 14/16 | 58/95 | 与 `v19` 同配方的显式脚本化复跑（20260527），headline 与当前 baseline / v19 持平 |
| v28 checkpoint-500 | `qwen3_vl_text` multiaux | 13/16 | 56/95 | 新 `maxaux8_20260528` balanced 10k 复跑（20260601），同配方下低于 v19 / v27 |
| v26 checkpoint-1000 | `qwen3_vl_text` multiaux | 12/16 | 52/95 | `v19` 同配置拉长到 1000 steps，完整评测后明显回退（20260525） |

补充说明：
- ddar bug fix（commit `ed0b877`）后，sft44 / v17 / v19 / v27 在 imo_95 上分别为 58/95、56/95、58/95、58/95；GRPO 相对 baseline 的 headline 优势在当前 commit 下消失。`v27` 与当前 baseline 虽然同为 `58/95`，但逐题 solved-set 有 2 题互换：`v27` 新增 `imo_sl_1999_g6`、`imo_sl_2020_g6`，同时丢失 `imo_sl_2013_g2`、`imo_sl_2016_g5_variant`。
- `v28` 使用新的 `maxaux8_20260528` 样本池衍生出的 `select_balanced` 10k 数据，在与 `v19/v27` 相同的 `grpo + group + token` 配方下最终为 `dev_imo 13/16`、`imo_95 56/95`；目前这次新样本池复跑没有带来收益。
- v19 历史最高曾达 `66/95`（旧 run），ddar bug fix 前最新完整补跑为 `65/95`（commit `6e24d512`）；这些历史结果保留在下方来源列表中。
- `v26` 说明继续把 `v19` 同配置从 `500` 步拉到 `1000` 步并不会带来更好的 benchmark 结果；虽然 KL 明显增大，但 `dev_imo` 和 `imo_95` 都回退。
- v17/v19 使用的是 `qwen3_vl_text` multiaux 评估链路；v18 历史结果来自更早的 `vlm` 链路，跨版本比较主要用于主线演进记录。
- v17 的 ddar bug fix 前结果（20260514）为 `dev_imo 14/16`、`imo_95 61/95`；bug fix 后下降到 `13/16` 和 `56/95`，与 sft44/v19 一致都呈现下移，可直接对齐比较。
...
## v26 长训练对照（v19 的 1000-step 版本）

`v26` 是把 `v19` 的训练步数从 `500` 直接拉到 `1000` 的对照实验，目的是观察"在同一组配置下继续训练 500 步能否进一步抬高 reward / KL 是否会失控"。除 `max_steps`（以及随之而来的 cosine scheduler 周期）以外，所有超参数与 `v19` 完全一致：

- 启动时间：`2026-05-25T05:17:25Z`，目录 `models/grpo_vlm_sft44_geometry100k_v26_s1_4gpu_lr5e6_1000steps/v0-20260525-051848`
- 共同配置：`grpo + group + token`，`lr=5e-6`，`beta=0.02`，`num_generations=8`，`temperature=1.1`，`max_completion_length=256`，dataset `geometry_clauses10_samples1M_select_balanced_10k.jsonl`，model `vlm_sft44/checkpoint-20084`
- v26 独有：`--max_steps 1000`（v19 为 500），cosine scheduler 因此衰减更慢
- 训练耗时：`5h 19m`，`train_speed≈19.2 s/it`，最终 `train_loss=0.008298`（v19 为 `2h 59m`，`21.5 s/it`，`train_loss=-0.001`）
- checkpoint：每 50 步存一份，最终保留到 `checkpoint-1000`

### 训练曲线对比（v19 vs v26）

按 step 抽取关键节点的指标：

| step | reward (v19) | reward (v26) | KL (v19) | KL (v26) | grad_norm (v19) | grad_norm (v26) | mean_len (v19) | mean_len (v26) | zero_std (v26) |
|------|--------------|--------------|----------|----------|-----------------|-----------------|----------------|----------------|----------------|
| 1    | 0.2383 | 0.2383 | 0.0000 | 0.0000 | 1.07 | 1.07 | 31.3 | 31.3 | 0.250 |
| 250  | 0.3391 | 0.3898 | 0.0060 | 0.0072 | 1.13 | 1.86 | 32.9 | 32.7 | 0.200 |
| 500  | **0.4711** | 0.4242 | **0.0051** | 0.0134 | 1.93 | 1.89 | 29.8 | 29.6 | 0.225 |
| 600  | — | **0.4773** | — | 0.0221 | — | 1.00 | — | 33.4 | 0.325 |
| 800  | — | 0.3797 | — | **0.0335** | — | 1.29 | — | 31.0 | 0.500 |
| 1000 | — | 0.4562 | — | 0.0262 | — | 0.89 | — | 32.2 | 0.425 |

### 下游评测结果（已完成）

| 版本 | dev_imo | imo_95 | 备注 |
|------|---------|--------|------|
| v19 checkpoint-500 | 14/16 | 58/95 | 当前 full benchmark 主线基线 |
| v26 checkpoint-1000 | 12/16 | 52/95 | `v19` 同配置延长到 1000 steps |

### 主要观察

- **reward**：v26 在 step 600 一度冲到 `0.4773`，但终点只剩 `0.4562`，仍低于 v19 在 step 500 的终点 `0.4711`。多训 500 步没有稳定抬高 reward 上限。
- **KL**：v26 的 KL 从 step 500 的 `0.0134` 继续升到 step 1000 的 `0.0262`，中途 step 800 还到过 `0.0335`；相对 v19 终点 `0.0051` 明显更大，说明策略持续偏离 reference。
- **零方差信号**：v26 的 `frac_reward_zero_std` 在后段明显恶化，step 800 到 `0.50`，step 1000 仍有 `0.425`；训练 reward 虽然不低，但样本区分度在下降。
- **梯度规模**：v26 的 grad_norm 并没有表现出“更稳地收敛”，step 500 仍在 `1.89`，后段只是随更长的 cosine 周期与更低 lr 一起波动下降。
- **completion 长度**：两者都稳定在 `~30-35` token，没有 v26 在更长训练下输出爆炸或塌缩的迹象。
- **训练速度**：v26 略快（`19.2 s/it` vs `21.5 s/it`），可能是不同时段集群负载差异，不是算法层面的因素。
- **下游评测**：完整评测已经确认 v26 回退到 `dev_imo 12/16`、`imo_95 52/95`，明显低于 v19 的 `14/16`、`58/95`，也低于当前的 sft44 baseline `58/95`。这说明这次 KL 变大不是“训练终于充分”的正信号，而是更接近 reward / 训练分布方向的偏移。

### 当前定位

- `v26` 的完整评测已经把问题定性为“继续训练带来泛化退化”，而不是“v19 只是训练不充分”。同配方拉长步数不是当前主线。
- 当前更合理的后续动作不是扫更多长训练 checkpoint，而是优先改 reward 目标或训练约束，例如降低 `valid_unsolved` 的奖励权重、提高 `beta`、或缩短到 `500-700` 步并尽早早停。
- 训练侧与 benchmark 侧合起来的结论是：**这组超参在 `500` 步附近已经是更好的停点**；继续训练主要表现为 KL 漂移增大，而不是求解能力提升。

主要证据：
- [v26 train log](/C20545/home/wangzi/GenesisGeo-grpo/models/grpo_vlm_sft44_geometry100k_v26_s1_4gpu_lr5e6_1000steps/train.log)
- [v26 run_metadata](/C20545/home/wangzi/GenesisGeo-grpo/models/grpo_vlm_sft44_geometry100k_v26_s1_4gpu_lr5e6_1000steps/run_metadata.json)
- [v26 trainer_state checkpoint-1000](/C20545/home/wangzi/GenesisGeo-grpo/models/grpo_vlm_sft44_geometry100k_v26_s1_4gpu_lr5e6_1000steps/v0-20260525-051848/checkpoint-1000/trainer_state.json)
- [v26 reward_breakdown](/C20545/home/wangzi/GenesisGeo-grpo/models/grpo_vlm_sft44_geometry100k_v26_s1_4gpu_lr5e6_1000steps/reward_breakdown.jsonl)
- [v26 dev_imo 评测](/C20545/home/wangzi/GenesisGeo-grpo/results/v26_lr5e6_checkpoint-1000_qwen3_vl_text_multiaux/eval_single_problem_multi_gpu_qwen3_vl_text_dev_imo_v0-20260525-051848_checkpoint-1000_sv1_auxfull_d32_b512_s4_gbs2_gbt100_seed123_20260525T120206Z.csv)
- [v26 imo_95 评测](/C20545/home/wangzi/GenesisGeo-grpo/results/v26_lr5e6_checkpoint-1000_qwen3_vl_text_multiaux/eval_single_problem_multi_gpu_qwen3_vl_text_imo_95_v0-20260525-051848_checkpoint-1000_sv1_auxfull_d32_b512_s4_gbs2_gbt100_seed123_20260525T140508Z.csv)
- [v19 train log（对照）](/C20545/home/wangzi/GenesisGeo-grpo/models/grpo_vlm_sft44_geometry100k_v19_s1_4gpu_lr5e6/train.log)
- [v19 dev_imo 评测（对照）](/C20545/home/wangzi/GenesisGeo-grpo/results/v19_lr5e6_checkpoint500_qwen3_vl_text_multiaux/eval_single_problem_multi_gpu_qwen3_vl_text_dev_imo_v0-20260508-105855_checkpoint-500_sv1_auxfull_d32_b512_s4_gbs2_gbt100_seed123_20260521T114949Z.csv)
- [v19 imo_95 评测（对照）](/C20545/home/wangzi/GenesisGeo-grpo/results/v19_lr5e6_checkpoint500_qwen3_vl_text_multiaux/eval_single_problem_multi_gpu_qwen3_vl_text_imo_95_v0-20260508-105855_checkpoint-500_sv1_auxfull_d32_b512_s4_gbs2_gbt100_seed123_20260521T133115Z.csv)

## v27 同配方复跑（500-step，显式脚本化）

`v27` 的目标不是再改训练目标，而是把 `v19` 这一条 `grpo + group + token` 主线用单独版本脚本显式固化下来，然后在当前仓库下重新完成一轮标准 benchmark。训练脚本 [tmp/train_v27.sh](/C20545/home/wangzi/GenesisGeo-grpo/tmp/train_v27.sh) 已明确注明“aligned with v19”，数据集、步数和主要超参数都与 `v19` 对齐：

- 数据集：`datasets/maxaux8/20260429/geometry_clauses10_samples1M_select_balanced_10k.jsonl`
- 模型目录：`models/grpo_vlm_sft44_geometry100k_v27_s1_4gpu_lr5e6/v0-20260527-030853/checkpoint-500`
- 配置族：`grpo + group + token`，`lr=5e-6`，`beta=0.02`，`num_generations=8`，`temperature=1.1`
- 训练步数：`500` steps，`save_steps=50`

**评估结果**：
- ✅ dev_imo：**14/16**
  - [v27 dev_imo CSV](/C20545/home/wangzi/GenesisGeo-grpo/results/v27_lr5e6_checkpoint500_qwen3_vl_text_multiaux/eval_single_problem_multi_gpu_qwen3_vl_text_dev_imo_v0-20260527-030853_checkpoint-500_sv1_auxfull_d32_b512_s4_gbs2_gbt100_seed123_20260527T065943Z.csv)
- ✅ imo_95：**58/95**
  - 初始 partial：[imo_95_resume_from_20260527_082737_partial.csv](/C20545/home/wangzi/GenesisGeo-grpo/results/v27_lr5e6_checkpoint500_qwen3_vl_text_multiaux/imo_95_resume_from_20260527_082737_partial.csv)（46/95）
  - remaining：[remaining CSV](/C20545/home/wangzi/GenesisGeo-grpo/results/v27_lr5e6_checkpoint500_qwen3_vl_text_multiaux/eval_single_problem_multi_gpu_qwen3_vl_text_imo_95_resume_from_20260527_082737_remaining_v0-20260527-030853_checkpoint-500_sv1_auxfull_d32_b512_s4_gbs2_gbt100_seed123_20260527T152852Z.csv)（12/49）
  - merged final：[imo_95_resume_from_20260527_082737_merged.csv](/C20545/home/wangzi/GenesisGeo-grpo/results/v27_lr5e6_checkpoint500_qwen3_vl_text_multiaux/imo_95_resume_from_20260527_082737_merged.csv)
- 评估脚本：
  - [tmp/eval_v27.sh](/C20545/home/wangzi/GenesisGeo-grpo/tmp/eval_v27.sh)
  - [tmp/resume_eval_v27_imo95.sh](/C20545/home/wangzi/GenesisGeo-grpo/tmp/resume_eval_v27_imo95.sh)

**结论**：
- `v27` 没有超过当前主口径：它与当前 sft44 baseline 和当前 `v19` 一样，都是 `dev_imo 14/16`、`imo_95 58/95`。
- 但它也不是“完全同一组 58 题”。相对当前 baseline，`v27` 多解了 `imo_sl_1999_g6`、`imo_sl_2020_g6`，少解了 `imo_sl_2013_g2`、`imo_sl_2016_g5_variant`。
- 这说明当前 headline 已经进入“同分但逐题组成波动”的区间；单次复跑 `58/95` 本身还不足以证明 `v19` 配方相对当前 baseline 存在稳定优势。

## v28 新样本池 balanced 10k 复跑

`v28` 不是继续改训练目标，而是在保持 `v19/v27` 同一套 `grpo + group + token` 训练配方不变的前提下，切换到新生成的 `maxaux8_20260528` 样本池衍生出的 balanced 10k 训练集，直接测试新数据源本身是否能带来稳定增益。

- 数据集：`datasets/maxaux8_20260528/20260529_grpo/geometry_clauses10_samples1M_select_balanced_10k.jsonl`
- 模型目录：`models/grpo_vlm_sft44_geometry100k_v28_s1_4gpu_lr5e6/v0-20260601-054854/checkpoint-500`
- 基座模型：`/C20545/home/wangzi/GenesisGeo_data_models/models/vlm_sft44/checkpoint-20084`
- 配置族：`grpo + group + token`，`lr=5e-6`，`beta=0.02`，`num_generations=8`，`temperature=1.1`
- 训练步数：`500` steps，`save_steps=50`

**评估结果**：
- ✅ dev_imo：**13/16**
  - [v28 dev_imo CSV](/C20545/home/wangzi/GenesisGeo-grpo/results/v28_lr5e6_checkpoint500_qwen3_vl_text_multiaux/eval_single_problem_multi_gpu_qwen3_vl_text_dev_imo_v0-20260601-054854_checkpoint-500_sv1_auxfull_d32_b512_s4_gbs2_gbt100_seed123_20260601T085350Z.csv)
- ✅ imo_95：**56/95**
  - [v28 imo_95 CSV](/C20545/home/wangzi/GenesisGeo-grpo/results/v28_lr5e6_checkpoint500_qwen3_vl_text_multiaux/eval_single_problem_multi_gpu_qwen3_vl_text_imo_95_v0-20260601-054854_checkpoint-500_sv1_auxfull_d32_b512_s4_gbs2_gbt100_seed123_20260601T103143Z.csv)

**结论**：
- `v28` 明确低于当前主线：相较 `v19/v27` 的 `14/16`、`58/95`，它回退到 `13/16`、`56/95`。
- 因为这次只换了训练数据来源而没有改训练配方，所以当前证据更支持“新 `maxaux8_20260528` balanced 10k 数据并未优于旧 `select_balanced` 10k 主线”，而不是训练脚本或评估链路的问题。
- 这也说明现阶段主线版本仍应保留 `v19/v27`，`v28` 更适合作为“新样本池复跑未带来提升”的反例记录。
...
- [benchmarks/imo95_mixed_outcomes_20.txt](/C20545/home/wangzi/GenesisGeo-grpo/benchmarks/imo95_mixed_outcomes_20.txt)
  ...
  - 包含 `20` 道在最初 8 个完整 run 里曾出现 solved / unsolved 翻转的题
...
这两个子集最初的分析来源固定为 `8` 个完整 `95/95` 的 `imo_95` run；后续新增的完整主线 run（如 `v27`）继续在下方登记为对照来源，但不改变这两个题集本身的定义。所有集合判断默认仍以最初那 8 个 run 的逐题 `Solved` 列为准，明确排除了 `partial` / `resume` 中间产物，以及 `2026-05-12` 那个只解出 `1/95` 的中断 run。具体来源如下：
...
- v17 multiaux
  ...
  - **最新结果 CSV（ddar bug fix 后，20260524，56/95）**：`results/v17_lr5e6_checkpoint500_qwen3_vl_text_multiaux/eval_single_problem_multi_gpu_qwen3_vl_text_imo_95_v0-20260423-165556_checkpoint-500_sv1_auxfull_d32_b512_s4_gbs2_gbt100_seed123_20260524T114315Z.csv`
  - 历史结果 CSV（ddar bug fix 前，20260514，61/95）：`results/v17_lr5e6_checkpoint500_qwen3_vl_text_multiaux/eval_single_problem_multi_gpu_qwen3_vl_text_imo_95_v0-20260423-165556_checkpoint-500_sv1_auxfull_d32_b512_s4_gbs2_gbt100_seed123_20260514T043538Z.csv`
  - eval commit（最新）：`42288dadcc0fc4f8cfc0e2de4068ae0dbaeccb6b`
...
- v27 multiaux
  - agent：`qwen3_vl_text`
  - model：`models/grpo_vlm_sft44_geometry100k_v27_s1_4gpu_lr5e6/v0-20260527-030853/checkpoint-500`
  - dev_imo CSV（20260527，14/16）：`results/v27_lr5e6_checkpoint500_qwen3_vl_text_multiaux/eval_single_problem_multi_gpu_qwen3_vl_text_dev_imo_v0-20260527-030853_checkpoint-500_sv1_auxfull_d32_b512_s4_gbs2_gbt100_seed123_20260527T065943Z.csv`
  - partial CSV（46/95）：`results/v27_lr5e6_checkpoint500_qwen3_vl_text_multiaux/imo_95_resume_from_20260527_082737_partial.csv`
  - remaining CSV（12/49）：`results/v27_lr5e6_checkpoint500_qwen3_vl_text_multiaux/eval_single_problem_multi_gpu_qwen3_vl_text_imo_95_resume_from_20260527_082737_remaining_v0-20260527-030853_checkpoint-500_sv1_auxfull_d32_b512_s4_gbs2_gbt100_seed123_20260527T152852Z.csv`
  - **merged 最终结果 CSV（58/95）**：`results/v27_lr5e6_checkpoint500_qwen3_vl_text_multiaux/imo_95_resume_from_20260527_082737_merged.csv`
- v28 multiaux
  - agent：`qwen3_vl_text`
  - model：`models/grpo_vlm_sft44_geometry100k_v28_s1_4gpu_lr5e6/v0-20260601-054854/checkpoint-500`
  - dev_imo CSV（20260601，13/16）：`results/v28_lr5e6_checkpoint500_qwen3_vl_text_multiaux/eval_single_problem_multi_gpu_qwen3_vl_text_dev_imo_v0-20260601-054854_checkpoint-500_sv1_auxfull_d32_b512_s4_gbs2_gbt100_seed123_20260601T085350Z.csv`
  - **imo_95 最终结果 CSV（56/95）**：`results/v28_lr5e6_checkpoint500_qwen3_vl_text_multiaux/eval_single_problem_multi_gpu_qwen3_vl_text_imo_95_v0-20260601-054854_checkpoint-500_sv1_auxfull_d32_b512_s4_gbs2_gbt100_seed123_20260601T103143Z.csv`
- v17 singleaux
  - agent：`qwen3_vl_text`
  - model：`models/grpo_vlm_sft44_geometry100k_v17_s1_4gpu_lr5e6/v0-20260423-165556/checkpoint-500`
...
- 原始数据源（版本相关）：
  ...
  - v19-v27：`datasets/maxaux8/20260429/geometry_clauses10_samples1M_select_balanced_10k.jsonl`
...
**主评估结果（multiaux / `qwen3_vl_text`，ddar bug fix 后的当前主口径）**：
- ✅ dev_imo：**14/16**
  - 最新完整重跑 CSV：`results/v19_lr5e6_checkpoint500_qwen3_vl_text_multiaux/eval_single_problem_multi_gpu_qwen3_vl_text_dev_imo_v0-20260508-105855_checkpoint-500_sv1_auxfull_d32_b512_s4_gbs2_gbt100_seed123_20260521T114949Z.csv`
- ✅ imo_95：**58/95**
  - 最新完整重跑 CSV：`results/v19_lr5e6_checkpoint500_qwen3_vl_text_multiaux/eval_single_problem_multi_gpu_qwen3_vl_text_imo_95_v0-20260508-105855_checkpoint-500_sv1_auxfull_d32_b512_s4_gbs2_gbt100_seed123_20260521T133115Z.csv`
...
- 说明：
  - ddar bug fix 后的当前主结果应以最新完整补跑 `58/95` 为准
  - ddar bug fix 前的最新完整补跑为 `65/95`
...
| v17 checkpoint-500 (`qwen3_vl_text`，ddar bug fix 后) | 13/16 | 56/95 | 20260524 重跑，eval commit `42288da` |
| v17 checkpoint-500 (`qwen3_vl_text`，ddar bug fix 前) | 14/16 | 61/95 | 20260514 multiaux 补跑，eval commit `6e24d512` |
...
| **v19 checkpoint-500 multiaux（ddar bug fix 后当前主口径）** | **14/16** | **58/95** | `select_balanced` 10k，20260521 最新完整重跑 |
| v19 checkpoint-500 multiaux（ddar bug fix 前） | 14/16 | 65/95 | 20260513 完整补跑，eval commit `6e24d512` |
| v19 checkpoint-500 multiaux（历史最佳 run） | 14/16 | 66/95 | 非当前 commit / 旧 run 的 merged 最佳结果 |

**与当前 pre-GRPO multiaux 基线对比（58/95 → 58/95，ddar bug fix 后）**：
- headline：当前主口径下与 baseline 持平，没有净增题数
- 历史上 bug fix 前的 `v19` 完整补跑曾到 `65/95`，相对旧 baseline `62/95` 有过 `+3` headline 提升
- 因此 `v19` 更适合被视为“曾经出现过更强历史 run，但在当前评估链路下与 baseline 持平”的主线版本

**关键分析结论**：
- v19 的主收益来自 **multiaux 搜索**，不是单 aux 本身；singleaux 对照只做到 `13/16` 和 `55/95`
- 在 ddar bug fix 后的当前主口径里，`dev_imo` 保持 `14/16`，`imo_95` 为 `58/95`，与当前 pre-GRPO multiaux baseline 持平
- 历史上同配置曾跑到 `65/95` 乃至 `66/95`，说明这条配方的上界曾更高，但当前可复现实验结论已经回落到与 baseline 同分

**结论**：
- v19 仍然是当前最值得保留的 `select_balanced` multiaux 主线之一：以 ddar bug fix 后的当前主口径计，`dev_imo 14/16`、`imo_95 58/95`，优于 v17 / v18，但不再高于当前 baseline
- singleaux 结果显著低于 multiaux，后续比较与回归时应继续把 multiaux 作为主汇报口径
- 旧 commit / 旧 run 的 `65/95` 与 `66/95` 应作为历史上界记录，而不是覆盖当前主结果
...
**主评估结果（`qwen3_vl_text` multiaux，当前 commit，ddar bug fix 后）**：
- ✅ dev_imo：**13/16**
  - `results/v17_lr5e6_checkpoint500_qwen3_vl_text_multiaux/eval_single_problem_multi_gpu_qwen3_vl_text_dev_imo_v0-20260423-165556_checkpoint-500_sv1_auxfull_d32_b512_s4_gbs2_gbt100_seed123_20260524T100004Z.csv`
- ✅ imo_95：**56/95**
  - `results/v17_lr5e6_checkpoint500_qwen3_vl_text_multiaux/eval_single_problem_multi_gpu_qwen3_vl_text_imo_95_v0-20260423-165556_checkpoint-500_sv1_auxfull_d32_b512_s4_gbs2_gbt100_seed123_20260524T114315Z.csv`
- 说明：
  - eval commit `42288da`，与 sft44（58/95）/ v19（58/95）的最新重跑使用同一评估链路，可直接对齐
  - ddar bug fix 前同评估链路结果为 `dev_imo 14/16`、`imo_95 61/95`（20260514，commit `6e24d512`），保留在历史结果中

**历史结果（旧 commit / 旧 run）**：
- `qwen3_vl_text` multiaux（ddar bug fix 前）：dev_imo **14/16**，imo_95 **61/95**（20260514）
...
| v16 checkpoint-500 | 14/16 | 55/95 | lr=5e-6，2k 数据集，8 epoch，非当前 commit（`a5482b00`） |
| **v17 checkpoint-500 (`qwen3_vl_text` multiaux，ddar bug fix 后)** | **13/16** | **56/95** | 当前 commit，eval commit `42288da`，与 sft44/v19 最新重跑同链路 |
| v17 checkpoint-500 (`qwen3_vl_text` multiaux，ddar bug fix 前) | 14/16 | 61/95 | 20260514 旧重跑，eval commit `6e24d512` |
...
- 以当前 commit、ddar bug fix 后的 `qwen3_vl_text` multiaux 结果计，v17 为 `dev_imo 13/16`、`imo_95 56/95`，落后 sft44 baseline（58/95）2 题，被 v19（58/95）超过
- ddar bug fix 前的对应结果是 `14/16`、`61/95`，bug fix 后整体下移与 sft44/v19 趋势一致
...
| v26 | `datasets/maxaux8/20260429/geometry_clauses10_samples1M_select_balanced_10k.jsonl` | `models/grpo_vlm_sft44_geometry100k_v26_s1_4gpu_lr5e6_1000steps/` |
| v27 | `datasets/maxaux8/20260429/geometry_clauses10_samples1M_select_balanced_10k.jsonl` | `models/grpo_vlm_sft44_geometry100k_v27_s1_4gpu_lr5e6/` |
| v28 | `datasets/maxaux8_20260528/20260529_grpo/geometry_clauses10_samples1M_select_balanced_10k.jsonl` | `models/grpo_vlm_sft44_geometry100k_v28_s1_4gpu_lr5e6/` |
...
- **v17**：与 v16 相同配置，训练集从 2k 扩大到 5k（约 3.2 epoch），前 50 步 avg_zero_std = 0.0778（优于 v16）；旧 `vlm` 链路结果为 59/95（非当前 commit，dev: `497d6aaf` / imo95: `e577945a`），当前 commit 的 `qwen3_vl_text` multiaux 在 ddar bug fix 前为 `14/16` `61/95`（20260514，eval commit `6e24d512`），ddar bug fix 后重跑为 `13/16` `56/95`（20260524，eval commit `42288da`）
- **v18**：将 bucket_unified 训练集从 5k 扩大到 10k，但 500 步只覆盖约 1.6 epoch，dev_imo 持平、imo_95 回退至 55/95（非当前 commit，评估 run commit: `f384d702`）
- **v19**：切换到 `select_balanced` 10k 数据集；ddar bug fix 后当前主口径的 `qwen3_vl_text` multiaux 为 `dev_imo 14/16`、`imo_95 58/95`，历史旧 run 曾到 `65/95` 与 `66/95`
...
- **v26**：与 `v19` 保持相同 `grpo + group + token` 配置，只把训练从 `500` 步拉长到 `1000` 步；训练 KL 显著增大，但完整评测回退到 `dev_imo 12/16`、`imo_95 52/95`，说明单纯加步数不是有效方向
- **v27**：把 `v19` 同配方用独立版本脚本显式固化后重新完整评测，结果为 `dev_imo 14/16`、`imo_95 58/95`；headline 与当前 baseline / v19 持平，但逐题 solved-set 相对 baseline 发生 2 题互换
- **v28**：保持 `v19/v27` 相同训练配方，只切换到新的 `maxaux8_20260528` balanced 10k 数据；最终 `dev_imo 13/16`、`imo_95 56/95`，低于旧主线，说明这次新样本池复跑没有带来提升
