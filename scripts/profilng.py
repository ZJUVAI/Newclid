...
for problem in problems:
    ...
    os.system(
        f"python -m cProfile -o profiling_exp/{problem}/profile.prof -s cumulative -m geosolver --problem-name {problem} --problems-file problems_datasets/imo_ag_30.txt --env profiling_exp --rules src/geosolver/default_configs/rules.txt"
    )
    os.system(
        f"python -m cProfile -o profiling_opt/{problem}/profile.prof -s cumulative -m geosolver --problem-name {problem} --problems-file problems_datasets/imo_ag_30.txt --env profiling_opt --rules src/geosolver/default_configs/rules_opt.txt"
    )
