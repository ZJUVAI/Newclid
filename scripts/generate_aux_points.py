...
DEFAULT_INPUT = "/c23474/home/duzhengtong/Discovery-GenesisGeo/datasets/rebuild_problems/imo_30_rebuild.txt"
DEFAULT_OUTPUT = "/c23474/home/duzhengtong/Discovery-GenesisGeo/datasets/rebuild_problems/imo_30_rebuild_aux_points.txt"
# DEFAULT_OUTPUT = "/c23474/home/duzhengtong/Discovery-GenesisGeo/datasets/rebuild_problems/imo_30_rebuild_aux_points_overlap.txt"
DEFAULT_TOLERANCE = 1e-6
DEFAULT_MAX_WORKERS = 10  # 默认单进程，设置 > 1 启用并行
BAN_OVERLAP = True  # 是否禁止生成与已有点重合的辅助点
# ==================== 日志配置 ====================
...
def merge_line_sources(line_sources: List[Tuple[str, str]], 
                       points: Dict[str, Tuple[float, float]], 
                       tol: float) -> List[Tuple[str, str]]:
    """
    合并表示同一条直线的多个线对。
    对于每组共线的线对，只保留字典序最小的那个作为代表。
    
    Args:
        line_sources: 线对列表，如 [('a', 'b'), ('c', 'd'), ...]
        points: 点名到坐标的映射
        tol: 数值容差
    ...
    for i in range(n):
        for j in range(i + 1, n):
            if is_same_line(line_sources[i], line_sources[j], points, tol):
                union(i, j)
    ...
    for i in range(n):
        ...
        if root not in groups:
            groups[root] = []
        groups[root].append(line_sources[i])
    
    # 每组选择字典序最小的作为代表
    result = []
    for group in groups.values():
        representative = min(group)  # 元组的字典序比较
        result.append(representative)
    
    return result


def merge_circle_sources(circle_sources: List[Tuple[Tuple[str, str, str], Tuple[Tuple[float, float], float]]], 
                         tol: float) -> List[Tuple[Tuple[str, str, str], Tuple[Tuple[float, float], float]]]:
    """
    合并表示同一个圆的多个圆定义。
    对于每组相同的圆，只保留第一个作为代表。
    
    Args:
        circle_sources: 圆定义列表
        tol: 数值容差
    
    Returns:
        去重后的圆定义列表
    """
    if len(circle_sources) <= 1:
        return circle_sources
    
    result = []
    for circle_info in circle_sources:
        _, (center, radius) = circle_info
        is_dup = False
        for existing_info in result:
            _, (ex_center, ex_radius) = existing_info
            if is_same_circle((center, radius), (ex_center, ex_radius), tol):
                is_dup = True
                break
        if not is_dup:
            result.append(circle_info)
    
    return result


def merge_sources(sources: List[Tuple[str, object]], 
                  points: Dict[str, Tuple[float, float]], 
                  tol: float) -> Tuple[List[Tuple[str, str]], List[Tuple[Tuple[str, str, str], Tuple[Tuple[float, float], float]]]]:
    """
    合并所有来源，将表示同一条线/同一个圆的多个来源合并。
    
    Args:
        sources: 来源列表，格式为 [('line', (p1, p2)), ('circle', circle_info), ...]
        points: 点名到坐标的映射
        tol: 数值容差
    ...
    Returns:
        (合并后的线列表, 合并后的圆列表)
    """
    line_sources = [obj for t, obj in sources if t == 'line']
    circle_sources = [obj for t, obj in sources if t == 'circle']
    
    merged_lines = merge_line_sources(line_sources, points, tol)
    merged_circles = merge_circle_sources(circle_sources, tol)
    
    return merged_lines, merged_circles
...
def generate_intersection_points(points: Dict[str, Tuple[float, float]], 
    ...
    注意：在筛选阶段会对来源进行合并去重，确保不同表示的同一条线/圆被视为同一来源。
    """
    aux_points = []
    ...
    # 注意：需要先合并表示同一条线/圆的不同来源
    for coord, data in spatial_hash.get_all():
        sources = data['sources']
        if len(sources) >= 3:
            # 合并同一条线/同一个圆的多个来源
            merged_lines, merged_circles = merge_sources(sources, points, tol)
            
            # 合并后的来源总数
            total_merged_sources = len(merged_lines) + len(merged_circles)
            
            # 只有合并后的来源数 >= 3 才添加
            if total_merged_sources >= 3:
                # 生成 predicate：优先用线
                objects_used = []
                for line in merged_lines[:2]:
                    objects_used.append(('line', line))
                if len(objects_used) < 2:
                    for circle_info in merged_circles[:2 - len(objects_used)]:
    ...
    for a_name in point_names:
        ...
        for line_cd in lines:
            ...
            for line_ef in lines:
                ...
                # EF不能与CD相同
                if point_on_line(c_coord, e_coord, f_coord, tol) or point_on_line(d_coord, e_coord, f_coord, tol):
                    continue
...
def format_predicate(aux_point: dict, aux_name: str) -> str:
    ...
    if ap_type == 'int':
        ...
        return f"perp {a_name} {aux_name} {c_name} {d_name}, cong {a_name} {c_name} {aux_name} {c_name}"
    
    elif ap_type == 'foot':
        # 垂足: perp a b c d, coll b c d, coll b e f
        a_name = aux_point['point']
        c_name, d_name = aux_point['line_cd']
        return f"perp {a_name} {aux_name} {c_name} {d_name}, coll {aux_name} {c_name} {d_name}"    
    return ""


def format_construction(aux_point: dict, aux_name: str) -> str:
    """生成辅助点的 construction 字符串"""
    ap_type = aux_point['type']
    
    if ap_type == 'int':
        # 交点: 根据 objects 类型生成不同的 construction
        objects = aux_point['objects']
        parts = []
        for obj_type, obj in objects:
            if obj_type == 'line':
                parts.append(f"on_line {aux_name} {obj[0]} {obj[1]}")
            else:  # circle
                circle_points = obj[0]
                parts.append(f"on_circum {aux_name} {circle_points[0]} {circle_points[1]} {circle_points[2]}")
        return f"{aux_name} = " + ", ".join(parts)
    
    elif ap_type == 'mid':
        # 中点: midpoint a b
        a_name, b_name = aux_point['segment']
        return f"{aux_name} = midpoint {aux_name} {a_name} {b_name}"
    
    elif ap_type == 'ref':
        # 镜像: reflect point line_p1 line_p2
        point_name = aux_point['point']
        c_name, d_name = aux_point['line']
        return f"{aux_name} = reflect {aux_name} {point_name} {c_name} {d_name}"
    
    elif ap_type == 'foot':
        # 垂足: foot point line_p1 line_p2
        point_name = aux_point['point']
        c_name, d_name = aux_point['line_cd']
        return f"{aux_name} = foot {aux_name} {point_name} {c_name} {d_name}"
    ...
    with open(output_path, 'w', encoding='utf-8') as f:
        for problem, aux_points in zip(problems, all_aux_results):
            ...
            for ap in aux_points:
                ...
                construction = format_construction(ap, aux_name)
