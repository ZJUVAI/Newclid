...
def ray_solve_problem(args):
    ...
    try:
        ...
        if agent_type == "lm":
            ...
            raise ValueError(
                f"Unknown agent type: {agent_type}. Must be 'lm' or 'qwen35_text'"
            )
...
def solve_problems(
    ...
    agent_type: str = "lm",
    log_dir: str = None,
    problem_db_root: str = "problem_db",
    enable_problem_db: bool = False,
):
    """
    ...
    pending_tasks = []
    problem_db_writer = (
        ProblemDBWriter(problem_db_root, repo_root=Path.cwd())
        if enable_problem_db
        else None
    )
    ...
    with Live(refresh_per_second=1) as live:
        while pending_tasks:
            ...
            done_tasks, pending_tasks = ray.wait(
                pending_tasks, num_returns=1, timeout=5
            )
            # Process completed tasks
            for task in done_tasks:
                pid, problem_name, is_solved, elapsed_time, problem_db_payload = (
                    ray.get(task)
                )
    ...
    with open(csv_filepath, "w", newline="", encoding="utf-8") as csvfile:
        ...
        writer.writerow(
            [
                f"Dataset: {dataset_name}, Solved: {solved_count}/{total_problems}, Total Time: {total_time:.2f}s"
            ]
        )
        ...
        for problem_name, status, elapsed_time in all_tasks_info:
            ...
            )  # Show time for processed problems, leave empty for pending
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Run Newclid evaluation with configurable paths."
    )
    parser.add_argument(
        "--problems_path",
        type=str,
        default="problems_datasets/dev_jgex.txt",
    ...
    parser.add_argument(
        "--max_workers", type=int, default=8, help="Number of worker processes to use"
    )
    ...
    parser.add_argument(
        "--timeout", type=int, default=7200, help="Timeout for each problem"
    )
    parser.add_argument(
        "--agent",
        type=str,
        default="lm",
        choices=["lm", "qwen35_text"],
