...
def main():
        ...
        for line in tqdm(f_in, desc="Updating JSONL"):
            ...
            try:
                ...
                if 'image_path' in data:
                    # 简单字符串替换，确保路径结构匹配
                    data['image_path'] = data['image_path'].replace(old_path_str, new_path_str)

                if 'image_path_no_annotations' in data:
                    # 简单字符串替换，确保路径结构匹配
                    data['image_path_no_annotations'] = data['image_path_no_annotations'].replace(old_path_str, new_path_str)
    ...
    print(f"JSONL processing complete.")
