...
def parse_eval_csv(csv_path: Path) -> tuple[str, int, int, float, list[list[str]]]:
    ...
    summary = rows[0][0]
    match = re.match(
        r"^Dataset: (?P<dataset>.*), Solved: (?P<solved>\d+)/(?P<total>\d+), Total Time: (?P<time>[0-9.]+)s$",
        summary,
    )
    if match is None:
    ...
    csv_rows = rows[2:]
    return dataset_name, solved_count, total_count, total_time, csv_rows
...
def upload_eval(args: argparse.Namespace) -> None:
    ...
    dataset_name, solved_count, total_count, total_time, csv_rows = parse_eval_csv(csv_path)
    ...
    if args.log_table:
        ...
        table.add(headers=["Problem Name", "Solved", "Time (s)"], rows=csv_rows)
