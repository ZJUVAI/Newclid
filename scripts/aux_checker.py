...
from newclid.configs import default_defs_path  # noqa: E402
from newclid.formulations.definition import DefinitionJGEX  # noqa: E402
from newclid import GeometricSolverBuilder, GeometricSolver  # noqa: E402
from newclid.predicates import NAME_TO_PREDICATE  # noqa: E402
from newclid.agent.lm import LMAgent  # noqa: E402
...
class AuxChecker:
    ...
    def __init__(self, seed: int = 123, max_attempts: int = 100):
        ...
        self.lm_agent = LMAgent(
            model_path=[], decoding_size=1, beam_size=1, search_depth=1
        )
    ...
    def remove_clauses_with_aux_points(
        self, fl_problem: str, aux_points: Set[str]
    ) -> str:
        ...
        # 分割子句
        clauses = [
            clause.strip() for clause in problem_part.split(";") if clause.strip()
        ]
        # 过滤掉包含aux点的子句
        filtered_clauses = []
                ...
                for line_num, line in enumerate(f, 1):
                    try:
                        ...
                        if not llm_output:
                            continue
                        # 提取aux内容
                        aux_contents = self.extract_aux_from_llm_output(
                            llm_output_renamed
                        )
                        if not aux_contents:
                            continue
                        ...
                        try:
                            # 解析aux内容
                            for aux_content in aux_contents:
                                aux_points, aux_premises = aux_content.split(";")[
                                    0
                                ].split(":")
                                ...
                                # 解析premises
                                aux_premises = re.split(
                                    r"\s*\[\d+\]", aux_premises.strip()
                                )
                                preparsed_premises = []
                                ...
                                # 使用lm.py的函数进行翻译
                                aux_clauses = self.lm_agent.try_dsl_to_constructions(
                                    aux_content
                                )
                                ...
                                for con in aux_constructions:
                                    ...
                                    if con_items[0] in self.defs:
                                        ...
                                        if len(con_items) == len(cdef.declare):
                                            mapping = dict(
                                                zip(cdef.declare[1:], con_items[1:])
                                            )
                                ...
                                preparsed_text = (
                                    aux_points + " " + ", ".join(preparsed_premises)
                                )
                                retranslated_text = (
                                    aux_points + " " + ", ".join(retranslated_premises)
                                )
                                preparsed_texts.append(preparsed_text)
                                retranslated_texts.append(retranslated_text)
                                missing_in_retranslated = (
                                    preparsed_set - retranslated_set
                                )
                                if missing_in_retranslated:
                                    ...
                                    log_entry += (
                                        f"llm_output_renamed: {llm_output_renamed}\n"
                                    )
                            ...
                            error_entry += (
                                f"原始premises: {'; '.join(preparsed_texts)}\n"
                            )
                            error_entry += (
                                f"重译premises: {'; '.join(retranslated_texts)}\n"
                            )
                            error_entry += (
                                f"原始llm_output_renamed:\n{llm_output_renamed}\n"
                            )
                            error_entry += "-" * 50 + "\n"
        ...
        try:
            with open(output_file, "w", encoding="utf-8") as f:
                ...
                if total_processed > 0:
                    f.write(
                        f"成功率: {(total_processed - inconsistencies_found) / total_processed:.2%}\n"
                    )
            ...
            with open(input_file, "r", encoding="utf-8") as f:
                for line_num, line in enumerate(f, 1):
                    try:
                        ...
                        # 移除包含aux点的子句
                        filtered_problem = self.remove_clauses_with_aux_points(
                            fl_problem, aux_points
                        )
    ...
    ) -> Tuple[bool, Optional[GeometricSolver]]:
        """解决单个几何问题"""
        try:
    ...
    def test_problems_from_list(self, problems: List[str]) -> Dict[str, int]:
        ...
        for i, problem in enumerate(problems, 1):
            ...
            if processed_problems % 100 == 0 or i == len(problems):
                print(
                    f"进度 {processed_problems}/{len(problems)}: 已求解={solved_problems}, 已构建={build_problems}, 已处理={processed_problems}"
                )
        results = {
    ...
    def run_full_pipeline(self, input_file: str) -> Dict[str, any]:
        ...
        if solve_results["total"] > 0:
            ...
            print(
                f"  求解成功: {solve_results['solved']} ({solve_results['solved'] / solve_results['total']:.1%})"
            )
            print(
                f"  构建成功: {solve_results['build']} ({solve_results['build'] / solve_results['total']:.1%})"
            )
...
def main():
    ...
    parser.add_argument(
        "--max-attempts", type=int, default=100, help="求解器最大尝试次数 (默认: 100)"
    )
    ...
    try:
        if args.command == "validate-translation":
            ...
            # 综合判断是否成功
            success = (
                results.get("translation_valid", False)
                and results.get("solve_results", {}).get("total", 0) > 0
            )
            sys.exit(0 if success else 1)
    except Exception as e:
        print(f"执行过程中发生错误: {e}")
