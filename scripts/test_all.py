...
def run_newclid_dataset(filepath: Path, dataset_name: str, output_file, timestamp: str):
    ...
    for problem_name in problem_names:
        problem_name, is_solved, elapsed_time, rule_match_stats = solve_problem(
            (problem_name, filepath)
        )
    ...
    output_file.write("\nSummary:\n")
    output_file.write(
        f"Solved: {solved_count}/{total_problems} ({solved_percentage:.2f}%)\n"
    )
    output_file.write(
        f"Failed: {failed_count}/{total_problems} ({100 - solved_percentage:.2f}%)\n"
    )
    ...
    if problem_rule_stats:
        ...
        generate_rule_time_chart(
            problem_rule_stats, dataset_name, charts_dir, timestamp
        )
...
def run_all_tests():
    ...
    with open(output_path, "w") as output_file:
        output_file.write("Newclid Test Results\n")
        output_file.write(
            f"Generated on: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n"
        )
        output_file.write("Command: python test_all.py\n")
        output_file.write("=" * 50 + "\n")

        for dataset_path, dataset_name in datasets:
            dataset_path = Path(dataset_path)
            solved, problems, comp_time, success_time, failed_time, rule_match_stats = (
                run_newclid_dataset(dataset_path, dataset_name, output_file, timestamp)
            )
            total_solved += solved
        ...
        overall_avg_time_all = (
            total_computation_time / total_problems if total_problems > 0 else 0
        )
        overall_avg_time_success = (
            total_success_time / total_solved if total_solved > 0 else 0
        )
        overall_avg_time_failed = (
            total_failed_time / total_failed if total_failed > 0 else 0
        )
...
def generate_rule_time_chart(problem_rule_stats, dataset_name, output_dir, timestamp):
    ...
    for i, problem in enumerate(problems):
        ...
        for rule_name, time_val in sorted_problem_rules:
            if time_val > 0:
                ...
                if (
                    percentage > 8
                ):  # Lower threshold: show rule name and percentage for >8%
                    ax.text(
                        left_position + percentage / 2,
                        i,
                        f"{short_rule_name}: {percentage:.1f}%",
                        va="center",
                        ha="center",
                        color="black",
                        fontsize=8,
    ...
    for rule_name, avg_percentage in avg_sorted_rules:
        if avg_percentage > 0:
            ...
            if avg_percentage > 8:  # Lower threshold to show more information
                ax.text(
                    left_position + avg_percentage / 2,
                    len(problems),
    ...
    ax.set_title(
        f"Runtime of Rules - {dataset_name}", fontsize=14, weight="bold", pad=20
    )
    ax.set_xlim(0, 100)
    ...
    sorted_avg_rules = sorted(rule_avg_data.items(), key=lambda x: x[1], reverse=True)[
        :15
    ]
    ...
    plt.savefig(
        chart_path, dpi=300, bbox_inches="tight", facecolor="white", pad_inches=0.1
    )
    plt.close()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Run Newclid evaluation on multiple datasets."
    )
    parser.add_argument(
        "--output_dir",
        type=str,
