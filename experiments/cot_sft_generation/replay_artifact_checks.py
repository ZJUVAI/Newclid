...
try:
    ...
    from .core.backtrace_schema import is_backtrace_generation_style
    ...
    from core.backtrace_schema import is_backtrace_generation_style  # type: ignore[no-redef]
...
def recheck_item_record(item_record: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if thinking:
        thinking_budget = None if (is_insight_generation_style(generation_style) or is_backtrace_generation_style(generation_style)) else compute_thinking_total_budget(plan_for_checks)
        thinking_ok, thinking_message = validate_thinking_response(
            thinking,
            point_coords={} if (generation_style == INSIGHT_TEXT_V1 or is_backtrace_generation_style(generation_style)) else point_coords,
            require_coord_tags=False,
            max_total_len=thinking_budget,
        )
        if thinking_ok and (generation_style == INSIGHT_TEXT_V1 or is_backtrace_generation_style(generation_style)):
