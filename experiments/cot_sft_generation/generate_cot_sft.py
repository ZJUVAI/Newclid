...
def low_level_equality_claim_lacks_symbolic_support(
    ...
    relation_text = step.get("approved_route_relation") or step.get("relation", "")
    relation_keywords = relation_text_keywords(relation_text)
    if triangle_relation_family(relation_text):
        return False
    if "equal" not in relation_keywords or relation_keywords & {
        "angle",
        "ratio",
        "similar",
        "parallel",
    ...
    for idx, support in enumerate(support_relations or []):
        ...
        if claim_segments:
            symbolic_segment_coverage.update(claim_segments & support_segments)
        if len(claim_points & support_points) >= min(2, len(claim_points)):
            symbolic_point_support_count += 1
        if support_triangle_family in {"similar", "congruent"} and len(claim_points & support_points) >= 3:
            symbolic_segment_coverage.update(claim_segments)
...
def select_symbolic_equality_support_refs(
    relation_text,
    support_catalog,
    point_names,
    max_supports=2,
    ...
    for item in support_catalog or []:
        ...
        support_segments = extract_relation_segment_tokens(relation)
        support_points = extract_point_mentions(relation, point_names)
        segment_overlap = len(support_segments & relation_segments) if relation_segments else 0
        point_overlap = len(support_points & relation_points)
        if relation_segments:
            if segment_overlap <= 0:
                continue
        elif point_overlap < min(2, len(relation_points)):
            continue
        candidates.append(
            (
                1 if relations_semantically_match(relation, relation_text, point_names) else 0,
                segment_overlap,
                point_overlap,
                1 if lowered_ref.startswith("bridge_chain[") else 0,
                1 if lowered_ref.startswith("visible_facts[") else 0,
...
def find_similarity_goal_tail_precheckpoint(
    ordered_route_relations,
    goal_finish,
    goal_tail_relations,
    known_points,
    ...
    if not local_support_relations:
        return ""
    ...
    existing_lower = {relation.lower() for relation in existing_relations}
    anchor_segments = extract_relation_segment_tokens(anchor_relation)
    anchor_points = extract_point_mentions(anchor_relation, known_points)
    goal_points = extract_point_mentions(normalized_goal_finish, known_points)
    ...
    for route_relation in reversed((ordered_route_relations or [])[:anchor_index]):
        ...
        if (
            ...
            continue
        route_keywords = relation_text_keywords(normalized_route_relation)
        route_triangle_family = triangle_relation_family(normalized_route_relation)
        if route_triangle_family not in {"similar", "congruent"} and not (
            route_keywords & {"angle", "ratio", "parallel", "perpendicular", "circle", "collinear"}
        ):
            continue
        ...
        route_segments = extract_relation_segment_tokens(normalized_route_relation)
        route_points = extract_point_mentions(normalized_route_relation, known_points)
        anchor_segment_overlap = len(route_segments & anchor_segments)
        anchor_point_overlap = len(route_points & anchor_points)
        goal_point_overlap = len(route_points & goal_points)
        if anchor_segment_overlap < 2 and anchor_point_overlap < 3:
            continue
        key = (
            1 if "angle" in route_keywords else 0,
            1 if "ratio" in route_keywords else 0,
            1 if route_triangle_family in {"similar", "congruent"} else 0,
            anchor_segment_overlap,
            anchor_point_overlap,
            goal_point_overlap,
            -len(normalized_route_relation),
            normalized_route_relation.lower(),
        )
...
def build_dossier_goal_tail_relations(
    ...
    similarity_precheckpoint_relation = find_similarity_goal_tail_precheckpoint(
        ordered_route_relations,
        normalized_goal_finish,
        goal_tail_relations,
        known_points,
    ...
    def clean_claim_steps(step_items, field_name, max_len, allow_empty=False, available_bridge_chain=None):
        ...
        for idx, step in enumerate(step_items):
            ...
            if low_level_equality_claim_lacks_symbolic_support(
                ...
                return False, (
                    f"{field_name}[{idx}].claim is missing a local non-coordinate equality support chain"
                ), None
            if congruent_step_lacks_noncoordinate_correspondence_support(
                step_contract,
                resolved_supports,
